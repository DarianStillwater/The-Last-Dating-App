...
CREATE POLICY "Match participants can view games"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM matches
      WHERE matches.id = question_games.match_id
      AND (matches.user1_id = auth.uid() OR matches.user2_id = auth.uid())
    )
  );
...
CREATE POLICY "Match participants can view game answers"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM question_games qg
      JOIN matches m ON m.id = qg.match_id
...
CREATE POLICY "Match participants can view voice messages"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM matches
      WHERE matches.id = voice_messages.match_id
      AND (matches.user1_id = auth.uid() OR matches.user2_id = auth.uid())
    )
  );
